...
sub default_options {
  ...
  return {
    ...
    gsnap_dir    => undef,
    hisat2_dir   => undef,
    star_dir     => $self->private_conf('ENSEMBL_STAR_DIR'),
    tophat2_dir  => undef,
...
sub resource_classes {
  ...
  return {
    ...
    'index_default' => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$index_mem.   ' -R "rusage[mem='.$index_mem.   ',tmp=16000] span[hosts=1]"'},
    'index_himem'   => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$index_himem. ' -R "rusage[mem='.$index_himem. ',tmp=16000] span[hosts=1]"'},
    'align_default' => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$align_mem.   ' -R "rusage[mem='.$align_mem.   ',tmp=16000] span[hosts=1]"'},
    'align_himem'   => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$align_himem. ' -R "rusage[mem='.$align_himem. ',tmp=16000] span[hosts=1]"'},
    'samtobam_mem'  => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$samtobam_mem.' -R "rusage[mem='.$samtobam_mem.',tmp=16000] span[hosts=1]"'},
    'merge_mem'     => {'LSF' => '-q ' . $self->o('queue_name') . ' -n '. ($threads + 1) .' -M '.$merge_mem.   ' -R "rusage[mem='.$merge_mem. ',tmp=16000] span[hosts=1]"'},
  }
}
