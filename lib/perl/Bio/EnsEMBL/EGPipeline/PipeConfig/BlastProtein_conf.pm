...
sub default_options {
  ...
  return {
    ...
    pipeline_name => $self->tagged_pipeline_name('blast_protein', $self->o('ensembl_release')),
    ...
    # generate bigPsl bigBEDs from blastx alignments
    blastx_bigPsl => 0,
...
sub pipeline_wide_parameters {
 ...
 return {
   ...
   'blastx_bigPsl'  => $self->o('blastx_bigPsl'),
 };
}
...
sub pipeline_analyses {
  ...
  return [
...

    {
      ...
      -flow_into       => WHEN(
                            '#blastp#' => ['DumpProteome'],
                            '#blastx# && #blastx_bigPsl#' => ['DumpGenomeAndChromSizes'],
                            '#blastx# && !#blastx_bigPsl#' => ['DumpGenome'],
    ...
    {
      -logic_name      => 'DumpGenomeAndChromSizes',
      -module          => 'Bio::EnsEMBL::Hive::RunnableDB::Dummy',
      -max_retry_count => 0,
      -rc_name         => 'normal',
      -flow_into       => {
    ...
    {
      -logic_name      => 'DumpChromSizes',
      -module          => 'Bio::EnsEMBL::EGPipeline::Common::RunnableDB::DumpChromSizes',
      -can_be_empty    => 1,
      -batch_size      => 10,
      -max_retry_count => 0,
      -parameters      => {
...

    {
      ...
      -flow_into         => {
                              '2->A' => ['GFF3Dump'],
                              'A->2' => ['JSONDescription'],
                              '2' => WHEN('#blastx# && #blastx_db_type# eq "otherfeatures" && #blastx_bigPsl#' =>
                                       ['AlignmentsToBigBed']),
                            },
    },
    ...
    {
      -logic_name      => 'AlignmentsToBigBed',
      -module          => 'Bio::EnsEMBL::Hive::RunnableDB::Dummy',
      -can_be_empty    => 1,
      -batch_size      => 10,
      -max_retry_count => 0,
      -flow_into       => {
                            '1' => ['AlignmentsToBigPsl'],
                          },
      -meadow_type     => 'LOCAL',
    },

    {
      -logic_name      => 'AlignmentsToBigPsl',
      -module          => 'Bio::EnsEMBL::EGPipeline::BlastAlignment::AlignmentToBigPsl',
      -can_be_empty    => 1,
      -batch_size      => 5,
      -analysis_capacity => 5,
      -max_retry_count => 1,
      -parameters      => {
                            ...
                            default_only_coords => 1,
                            results_dir         => catdir($self->o('pipeline_dir'), '#species#'),
                            out_file_stem      => '#logic_name_prefix#_blastx.bigPsl',
                          },
      -flow_into       => {
                            '1' => ['BigPslToBigBed'],
                          },
    ...
    {
      -logic_name      => 'BigPslToBigBed',
      -module          => 'Bio::EnsEMBL::EGPipeline::Common::Converters::BigPslToBigBed',
      -can_be_empty    => 1,
      -batch_size      => 10,
      -analysis_capacity => 10,
      -max_retry_count => 0,
      -parameters      => {
                            ...
                            bigPsl_file    => catdir($self->o('pipeline_dir'), '#species#', '#species#.#logic_name_prefix#_blastx.bigPsl'),
                            chrom_sizes_file    => catdir($self->o('pipeline_dir'), '#species#', '#species#.chrom_sizes.tsv'),

                            results_dir   => catdir($self->o('pipeline_dir'), '#species#'),
                            out_file_stem => '#logic_name_prefix#_blastx.bb',
                          },
      -rc_name         => 'normal',
    },
