...
sub default_options {
  ...
  return {
    ...
    exonerate_2_4_dir    => $self->private_conf('ENSEMBL_EXONERATE_SERVER_DIR'),
...
sub resource_classes {
  ...
  return {
    ...
    'server' => {'LSF' => '-q ' . $self->o('queue_name') . ' -n ' . ($max_exonerate_jobs + 1) . ' -M ' . $server_memory . ' -R "rusage[mem=' . $server_memory . ']"'},
  }
}
