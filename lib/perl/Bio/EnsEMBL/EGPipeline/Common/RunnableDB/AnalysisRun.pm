...
sub param_defaults {
  ...
  return {
    ...
    'querylocation'    => undef,
    'queryfile'        => undef,
    'bindir'           => $self->private_conf('ENSEMBL_BINDIR'),
    'libdir'           => $self->private_conf('ENSEMBL_LIBDIR'),
    'datadir'          => $self->private_conf('ENSEMBL_DATADIR'), 
    'workdir'          => $self->private_conf('ENSEMBL_WORKDIR'),
    'parameters_hash'  => {},
    'results_index'    => 'slice',
    'parse_filehandle' => 0,
    'output_not_set'   => 0,
...
sub save_to_db {
  ...
  foreach my $feature (@{$runnable->output}) {
    ...
    if($self->param('save_object_type') eq 'ProteinFeature'){
    	$adaptor->store($feature,$feature->translation_id);    
    }else{
