...
sub production_dba {
  ...
  if (!defined $dba and $self->param_exists('production_db')) {
    my %production_db = %{$self->param('production_db')};
    $dba = Bio::EnsEMBL::DBSQL::DBAdaptor->new(%production_db);
  }
  if (!defined $dba and $self->param_exists('prod_db')) {
    my %production_db = %{$self->param('prod_db')};
    $dba = Bio::EnsEMBL::DBSQL::DBAdaptor->new(%production_db);
  }
  if (!defined $dba) {
    $self->throw("Could not get the production database from the registry or from a parameter");
  }
  confess('Type error!') unless($dba->isa('Bio::EnsEMBL::DBSQL::DBAdaptor'));
